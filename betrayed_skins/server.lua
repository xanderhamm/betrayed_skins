...
RegisterServerEvent("betrayed_skins:GetSex4")
AddEventHandler("betrayed_skins:GetSex4",function(source, cb)
    local src = source
...
RegisterServerEvent("betrayed_skins:GetSex5")
AddEventHandler("betrayed_skins:GetSex5",function(source, cb)
    MySQL.Async.fetchAll("SELECT sex FROM users WHERE identifier = @identifier", {["identifier"] = identifier}, function(result)
        if result ~= nil then
            --print(result)
            for k,v in pairs(result) do
                print(k,v)
